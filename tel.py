...
client = TelegramClient(phone_number, os.getenv("api_id"), os.getenv("api_hash"))
...
async def get_group_messages():
    ...
    for group_name, group_id in CHANNEL_NAMES.items():
        ...
        if group:
            ...
            since_date = date_today - timedelta(
                days=7
            ...
            if len(data) > 0:
                ...
                df["date"] = (
                    df["date"].dt.tz_convert("Asia/Singapore").dt.tz_localize(None)
                )
                ...
                df.to_excel(
                    f"{dir_name}/{group_name} {date_today.strftime('%Y-%m-%d')}.xlsx",
                    index=False,
                )
...
# subject = "Tel data" ## uncomment it for DSG tracking
# subject = f"Telegram data {datetime.now().strftime('%Y-%m-%d')}"
